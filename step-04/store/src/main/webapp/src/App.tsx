...
const MOCK_INVENTORY = [
  { projectName: 'Spring Boot',       type: 'T-Shirt',  price: 29.99, logoUrl: 'https://spring.io/img/spring-boot.svg' },
  { projectName: 'Spring Framework',  type: 'T-Shirt',  price: 29.99, logoUrl: 'https://spring.io/img/spring-framework.svg' },
  { projectName: 'Spring Cloud',      type: 'Sticker',  price:  4.99, logoUrl: 'https://spring.io/img/spring-cloud.svg' },
  { projectName: 'Spring Security',   type: 'Socks',    price: 14.99, logoUrl: 'https://spring.io/img/spring-security.svg' },
  { projectName: 'Spring Data',       type: 'Sticker',  price:  4.99, logoUrl: 'https://spring.io/img/spring-data.svg' },
  { projectName: 'Spring AI',         type: 'T-Shirt',  price: 34.99, logoUrl: 'https://spring.io/img/spring-ai.svg' },
  { projectName: 'Spring Batch',      type: 'Socks',    price: 14.99, logoUrl: 'https://spring.io/img/spring-batch.svg' },
]

function buildMockOrder() {
  const orderId = `order-${crypto.randomUUID().slice(0, 8)}`
  // Pick 1–3 random items
  const shuffled = [...MOCK_INVENTORY].sort(() => Math.random() - 0.5)
  ...
  return { orderId, items, totalPrice: Math.round(totalPrice * 100) / 100 }
}

function MockWorkflowButton({ theme }: { theme: ThemePalette }) {
  const [hovered, setHovered] = useState(false)
  const [status, setStatus] = useState<'idle' | 'loading' | 'ok' | 'error'>('idle')
  const isRetro = theme.name === 'retro'
  ...
  const label = status === 'loading' ? '...' : status === 'ok' ? '✓ Started' : status === 'error' ? '✗ Error' : 'Mock Workflow'
  ...
  const retroChrome = (
    <>
      ...
      <div style={{
        ...
        <MockWorkflowButton theme={theme} />
